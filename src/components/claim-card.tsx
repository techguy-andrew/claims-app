...
export function ClaimCard({ claimNumber, clientName, insuranceCompany, status, onClick }: ClaimCardProps) {
  return (
    <div 
      onClick={onClick}
      className="group enterprise-card p-6 md:p-4 mb-6 md:mb-4 cursor-pointer 
                 hover:shadow-[0_8px_40px_rgb(0,0,0,0.12)] transition-all duration-200 hover:-translate-y-1
                 active:scale-98 touch-target-lg min-h-[120px] md:min-h-[100px]"
    >
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 mb-5 md:mb-4">
            ...
            <h3 className="text-xl md:text-lg font-semibold text-gray-900 tracking-tight leading-tight">{claimNumber}</h3>
          </div>
          ...
          {/* Client Name */}
          <p className="text-base font-medium text-gray-800 ml-16 leading-snug">{clientName}</p>
        </div>
        
        {/* Arrow indicator - Mobile optimized */}
        <div className="touch-target-lg flex items-center justify-center min-h-[48px] min-w-[48px] md:min-h-[44px] md:min-w-[44px]">
      ...
      <div className="mt-6 md:mt-4 ml-16">
        <span className={`inline-flex items-center px-4 py-2.5 md:py-2 rounded-xl text-sm md:text-xs font-medium text-white bg-gradient-to-r ${statusGradients[status]} shadow-lg transition-all duration-200 hover:shadow-xl min-h-[36px] md:min-h-[28px]`}>
          {statusLabels[status]}
        </span>
      </div>
    </div>
